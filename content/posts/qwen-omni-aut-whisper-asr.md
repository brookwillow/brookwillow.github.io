---
title: "Bridging ASR Text Decoding from Omni AUT High-Dim Representations"
date: 2026-05-31T22:36:21+08:00
tags: ["AI", "语音"]
draft: false
---
车载语音助手里，端到端 Omni 模型有一个很吸引人的方向：用户说话后，不再先跑独立 ASR 再把文本送进大模型，而是直接让多模态模型从音频输入生成工具调用或自然语言回复。这样做的好处很明显——减少级联系信息损失，语音里的停顿、语气和情绪等上下文也能被模型利用。

但端到端也有一个很现实的短板：如果模型直接吃音频输出工具调用，中间就没有一份稳定的 ASR 文本。没有 ASR 文本，后续很多工程能力就会变得困难——日志排查、长期记忆落盘、RAG 预召回、用户偏好抽取、错误样本归因，全都需要一个"用户到底说了什么"的文本作为依据。
...
后续实验已经跑到了第三个阶段：直接拼接仍然不可靠，但选对 AUT 层 + 训练一个轻量 bridge 之后，旁路 ASR 已经出现了可用雏形。
...
- **日志审计**需要知道用户说了什么，不能只存音频和最终 JSON。
- **长期记忆**需要抽取事实和偏好，比如"用户喜欢主驾座椅偏后"。
- **RAG 预召回**通常依赖文本 query，音频直接召回成本更高。
- **训练数据闭环**需要定位错误来源——是 ASR 听错了，还是理解、工具选择或参数填充出了问题。
- **多轮省略表达**如"再关掉吧""调低一点"，也需要文本化历史才能分析。

最直接的办法是旁路再跑一个独立 Whisper。服务里确实保留过 `--debug-asr` 调试模式，用本地 tiny Whisper 帮助排查。但这只是调试辅助，没有复用端到端模型内部已经算好的音频表征。我们想验证的是：既然 Qwen2.5-Omni 已经有 AUT，能不能直接复用它的输出？
...
python scripts/probe_asr_decoder.py \
  --model-dir Qwen2.5-Omni-3B \
  --whisper-dir openai/whisper-large-v3 \
  --audio data/eval/audio/window/window_001.wav
```

脚本做的事：加载 Omni 模型 → 找到 `thinker.audio_tower` → 在 `audio_tower.ln_post` 上注册 forward hook → 跑一次推理捕获音频 hidden states → 包装成 Whisper 的 `encoder_outputs` → 调 Whisper decoder 做 transcribe。

选择 `ln_post` 的原因是它的 hidden size 正好是 **1280**，和 Whisper large-v3 的 `d_model=1280` 完全对齐。形状上没毛病：
...
- **训练目标**：Whisper encoder 服务于 ASR，Qwen AUT 在 Omni 里服务于多模态理解和后续 Thinker 推理。
...
另外有一个容易混淆的点：Qwen2.5-Omni 的 AUT 确实跟 Whisper 有很强关系，参数很可能来自 Whisper 初始化。所以直接接 Whisper decoder 偶尔能出字——这是"表征亲缘性"的体现。但初始化相同不代表训练结束后仍然兼容。Omni 训练把 AUT 表征推向了更适合 Thinker 消费的空间，而不是保持 Whisper encoder 的 ASR 条件分布。

## 直接解码的效果：能出字，但不稳定

实际跑 eval 里的 window 音频，出现了几类典型现象：
...
第一版选 `ln_post` 纯粹因为维度刚好。后来给 probe 脚本加了 `--print-audio-shapes`，把 AUT 内部各层的 shape 都打了出来：
...
关键发现：`ln_post` 已经过了 `avg_pooler`，时间步从 72 被压缩到了 36。而 `audio_tower.layers.31` 的输出是 `[72, 1280]`——时间步翻倍，维度不变。

于是补了一个 `--hook-module` 参数，可以 hook 任意层。把 hook 从 `ln_post` 切到 `layers.31` 后，直接接 Whisper decoder 的结果仍然是退化（`cakescakes...éta...`）。这说明了一个更深层的问题：**帧数对齐和 hidden size 对齐只是入场券，真正难点始终是表征分布对齐。**即使给你更多帧，decoder 不认识的分布照样不认识。

## Bridge：只训练一个小映射层

既然直接拼不行，下一步就是加一个轻量映射层。思路很克制：Qwen 和 Whisper 全部冻结，只训练一个 `AutAsrBridge`，把 AUT hidden space 映射回 Whisper decoder 更熟悉的 encoder space。
...
有了 shape probe 的结论后，把训练入口也支持了 `--hook-module`，用 `layers.31` 重新训练。当前效果最好的一组配置：
...
关键变化：hook 从 `ln_post [36,1280]` 换成 `layers.31 [72,1280]`，`repeat_factor=1`（不再硬补时间），bridge hidden dim 提到 2048。
...
1. 把 `layers31_e7_h2048` 固化为 baseline，记录训练命令、checkpoint 和 eval 指标，避免后续不可复现。
...
5. 尝试更强的时间 resampler——在 `repeat_factor=1` baseline 上试小型 Conv、learned upsampling 或 cross-attention resampler。
